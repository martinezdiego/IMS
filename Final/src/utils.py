...
def build_carrers_map(path):
    carrers_map = dict()
    data = pd.read_excel(path)
    for (_index, row) in data.iterrows():
        carrers_map[row['Carrera']] = [row['Pensum'], row['Asignaturas'
                ]]
    return carrers_map
...
def compute_min_semesters(max_courses, pensum):
    courses_map = build_courses_map('./db/' + pensum[1] + '.xlsx')
    N = len(courses_map)
    (graph, edges) = build_pensum(courses_map, './db/' + pensum[0]
                                  + '.xlsx')
...
def get_variables(data):
    ...
    carrers = []
    for (_index, row) in data.iterrows():
        carrers.append(row['Carrera'])
        Estudiantes.append(row['Nombre'])
    return (num, Estudiantes, carrers)
...
def output(
    ...
    for i in range(0, TOT_ESTUDIANTE):
        ...
        Dictionary['ESTUDIANTE_CARRERA'].append(ESTUDIANTE_CARRERA[i])
